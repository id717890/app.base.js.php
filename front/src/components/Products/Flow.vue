      ...
      <v-flex xs12 sm10 offset-sm1 md8 offset-md2>
        <v-layout row wrap>
          <v-flex xs12 sm12 md10 offset-md1 lg8 offset-lg2 text-xs-center>
            <h1>
              Фитнес-программа «ДВИЖЕНИЕ»
              <br />с элементами FLOW
            </h1>
          ...
          <v-flex xs12 sm12 md10 offset-md1 lg8 offset-lg2 text-xs-center mt-4>
            <div class="video-wrapper">
              <iframe
                src="https://drive.google.com/file/d/163CTVp4_ibMPMW6zN8t0yHqLQ3fmh_eK/preview"
                width="100%"
                height="100%"
              ></iframe>
            ...
            >
              ...
              <ul>
                <li>
                  <a
                    class="red--text"
                    href="https://support.google.com/accounts/answer/32050?co=GENIE.Platform%3DDesktop&hl=ru&oco=1"
                    target="_blank"
                  >Google Chrome</a>
                </li>
                <li>
                  <a
                    class="red--text"
                    href="https://firefox-browsers.ru/udalit-kuki-v-mozilla-firefox.html"
                    target="_blank"
                  >Mozilla Firefox</a>
                </li>
                <li>
                  <a
                    class="red--text"
                    href="https://operaru.ru/faq/how-to-clear-cookies-in-opera"
                    target="_blank"
                  >Opera</a>
                </li>
                <li>
          ...
          <v-flex
            ...
            offset-md1
            py-5
            v-if="product !== null && product !== undefined && product !== 'undefined' && product.product !== null && product.product !== undefined && product.product !== 'undefined' && product.days !== null"
          >
            <v-expansion-panel expand v-if="trainings !== null">
              <v-expansion-panel-content v-for="item in trainings" :key="item.id">
                <template v-slot:header>
                  <div @click="clickToHeader(item)">
                    <i :class="item.icon+' mr-3 ' + item.color+'--text'"></i>
                    {{item.title}}
                  </div>
                </template>
                <v-card class="grey lighten-4 text-xs-center">
                  <v-card-text>
                    <v-progress-linear
                      v-if="item.loading"
                      :indeterminate="true"
                      :ref="'myloader' + item.id"
                    ></v-progress-linear>
                    <div :ref="'videowrapper_' + item.id"></div>
                    <!-- <h2 v-else>Здесь будет видео</h2> -->
                  </v-card-text>
                  <v-card-text v-if="item.url_file !== null">
                    <a :href="item.url_file">
                      <i class="far fa-file-pdf"></i>
                      Скачать файл "{{item.title}}"
                    </a>
                  </v-card-text>
                </v-card>
    ...
    <v-flex xs12 pa-0 ma-0 v-else>
      <v-layout row wrap>
        ...
        <v-flex
          ...
          text-xs-center
          mt-4
          v-if="isAuth && product.is_available !== null  && product.is_available !== undefined  && product.is_available !== 'undefined'  && product.is_available === true"
        >
        ...
        <v-flex
          ...
          text-xs-center
          mt-4
          v-if="product && product.is_available === null  || product.is_available === undefined || product.is_available === 'undefined' || product.is_available === false"
        >
        ...
        <v-flex xs12 sm10 offset-sm1 md8 offset-md2 lg6 offset-lg3 text-xs-center mt-4>
          <video-player
            class="video-player-box video-intro video-wrapper"
            ref="videoPlayer"
        ...
        <v-flex
          ...
          offset-lg3
          py-5
          v-if="product !== null && product !== undefined && product !== 'undefined' && product.product !== null && product.product !== undefined && product.product !== 'undefined'"
        >
          <h3 class>Заполняя следующую форму регистрации, вы даете согласие:</h3>
          <ul class>
            <li>На обработку своих персональных данных</li>
          ...
          >
            ...
            <v-flex xs12>
              ...
              <input
                type="hidden"
                name="label"
                :value="user+'@@@'+product.product.id + '@@@'+selectedPrice"
              />
            ...
            <v-flex
              xs12
              text-xs-center
              v-if="product && product.is_available !== null  && product.is_available !== undefined  && product.is_available !== 'undefined'  && product.is_available === true"
            >
              <section v-if="isAuth">
                <v-layout row nowrap justify-center>
                  <v-select
            ...
            this.$refs['videowrapper_' + item.id][0].innerHTML =
              `
                <div  class="video-wrapper">
                    <iframe src="` +
              source.googledisk +
              `"   width="100%" height="100%" frameborder="0" scrolling="no" seamless="" allowfullscreen="1"></iframe>
                    <div class="hidedrive">&nbsp;</div>
                </div>`
          } else {
            this.$refs['videowrapper_' + item.id][0].innerHTML =
              `<div  class="video-wrapper">` + source.yadisk + `</div>`
          }
          setTimeout(() => {
