<template>
  <v-layout row wrap mb-4>
    ...
    <v-flex
      ...
      text-xs-center
      mt-4
      v-if="isAuth && product && product.is_available === true"
    >
    ...
    <v-flex xs12 sm10 offset-sm1 md8 offset-md2 lg6 offset-lg3 text-xs-center mt-4>
      ...
      <div class="video-wrapper">
        <iframe
          src="https://www.youtube.com/embed/YOocy6RUfsw"
          showinfo="0"
          controls="0"
          modestbranding="1"
          width="100%"
      ...
      >
        ...
        <v-flex xs12>
          ...
          <v-checkbox
            v-model="form.accept"
            :rules="acceptRules"
            label="Даю согласие на хранение и обработку моих персональных данных"
            required
          ></v-checkbox>
        </v-flex>
        <v-flex
          xs12
          text-xs-center
          v-if="product && product.is_available && product.is_available === true"
        >
          <section v-if="isAuth">
            <v-layout row nowrap justify-center>
              <v-select
...
export default {
  ...
  data() {
    return {
      ...
      fieldRules: [v => !!v || 'Поле обязательно для заполнения'],
      acceptRules: [
  ...
  methods: {
    ...
    buy(e) {
      ...
      if (this.$refs.form.validate()) {
        ...
        this.acceptProductWithPrice(this.form).then(() =>
